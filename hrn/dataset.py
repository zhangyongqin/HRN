...
class VerDataset(data.Dataset):
    def __init__(self, path, size, scale):
        super(VerDataset, self).__init__()

        self.size = size
        h5f = h5py.File(path, "r")
        
        self.hr = [v[:] for v in h5f["HR"].values()]
        # perform multi-scale valid
        if scale == 0:
            self.scale = [2, 3, 4]
            self.lr = [[v[:] for v in h5f["X{}".format(i)].values()] for i in self.scale]
        else:
...
class TestDataset(data.Dataset):
    def __init__(self, dirname, scale):
        ...
        if "" in self.name:
            if scale==3:
                self.hr = glob.glob(os.path.join("{}_HRx3".format(dirname), "*.png"))
            elif scale==2 :
                self.hr = glob.glob(os.path.join("{}_HRx2".format(dirname), "*.png"))
            elif scale==8 :
                self.hr = glob.glob(os.path.join("{}_HRx8".format(dirname), "*.png"))
            else:
                self.hr = glob.glob(os.path.join("{}_HRx4".format(dirname), "*.png"))
    ...
    def __getitem__(self, index):
        ...
        #hr, lr = random_crop(hr, lr, 48, self.scale)
